...
pub fn part_one(input: &str) -> Option<u64> {
    fn map_resources(contents: &str) -> (Vec<u64>, Vec<HashMap<u64, (u64, u64)>>) {
        ...
        for (i, line) in contents.lines().enumerate() {
            ...
            if i == 0 {
                ...
                let dst_start = mapping_line[0];
                let src_start = mapping_line[1];
                ...
                resource_mappings
                    ...
                    .last()
                    .unwrap()
                    .insert(src_start, (dst_start, range));
            } else {
    ...
    for seed in seeds {
        ...
        for (i, resource) in resource_mappings.iter().enumerate() {
            ...
            if i == resource_mappings.len() - 1 && key < minimum_location {
                minimum_location = key;
            }
        }
    }
    Some(minimum_location)
}
pub fn part_two(input: &str) -> Option<i64> {
    ...
    for (i, line) in input.lines().enumerate() {
        ...
        if line.chars().next().unwrap().is_numeric() {
            ...
            let mapping_difference = dst_start - src_start;
    
            let mapping_range = RangeSet {
                ranges: vec![src_start..src_start + range],
            };
