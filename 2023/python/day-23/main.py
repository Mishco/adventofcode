...
def get_neighbors(
    position: Pos, trail_map: list[str], slopes: bool = True
) -> Iterator[Pos]:
    ...
    for dx, dy in directions[trail_map[y][x] if slopes else "."]:
        ...
        if (
            0 <= new_x < width
            and 0 <= new_y < height
            and trail_map[new_y][new_x] != "#"
        ):
            yield new_x, new_y


def is_dead_end(
    position: Pos, prev: Pos, neighbors: list[Pos], trail_map: list[str]
) -> bool:
...
def get_graph(
    trail_map: list[str], slopes: bool = True
) -> dict[Pos, list[tuple[Pos, int]]]:
    vertices = [(1, 0)]
    visited = set()
    res_graph = {}
    while vertices:
...
def iter_hike_lengths(
    in_graph: dict[Pos, list[tuple[Pos, int]]], in_goal: Pos
) -> Iterator[int]:
    start = (1, 0)
    stack = [(start, 0, {start})]
