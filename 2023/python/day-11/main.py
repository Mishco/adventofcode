...
def test():
    ...
    t = sum(
        manhattan(g1, g2) for g1, g2 in combinations(expand(2, galaxies, cols, rows), 2)
    )
...
def calculate(distance_mult=1) -> int:
    ...
    for i in range(len(galaxies)):
        for j in range(i + 1, len(galaxies)):
            total += abs(galaxies[i][0] - galaxies[j][0]) + distance_mult * len(
                expand_rows[
                    (min(galaxies[i][0], galaxies[j][0]) < expand_rows)
                    & (max(galaxies[i][0], galaxies[j][0]) > expand_rows)
                ]
            )
            total += abs(galaxies[i][1] - galaxies[j][1]) + distance_mult * len(
                expand_cols[
...
if __name__ == "__main__":
    ...
    part2 = sum(
        manhattan(g1, g2)
        for g1, g2 in combinations(expand(1000000, galaxies, cols, rows), 2)
    )
    print(part2)
